...
pub const WebSocketConnection = struct {
    ...
    pub fn performHandshake(self: *WebSocketConnection, url: []const u8) !void {
        ...
        _ = try zsync.spawn(self.runtime, pingPongLoop, .{self});
    }
...
pub const WebSocketServer = struct {
    ...
    pub fn listen(self: *WebSocketServer, address: transport.Address) !void {
        ...
        // Start accept loop
        _ = try zsync.spawn(self.runtime, acceptLoop, .{self});
    }
    ...
    fn acceptLoop(self: *WebSocketServer) void {
        while (true) {
            ...
            switch (tcp_conn) {
                .ready => |result| {
                    if (result) |conn| {
                        _ = zsync.spawn(self.runtime, handleConnection, .{ self, conn }) catch continue;
                    } else |_| {
                        continue;
                    }
                },
